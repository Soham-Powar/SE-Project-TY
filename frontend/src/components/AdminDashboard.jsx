	...
	useEffect(() => {
		const fetchApplications = async () => {
			try {
				...
				if (!res.ok) {
					throw new Error("Unauthorized or failed to fetch applications");
				}
					...
					<tbody>
						{applications.map((app) => (
							<tr key={app.id} className="hover:bg-gray-50">
								...
								<td className="border p-3">{app.firstname} {app.lastname}</td>
								...
								<td className="border p-3 font-medium capitalize text-blue-600">{app.admission_status}</td>
